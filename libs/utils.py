...
def get_extra_ip():
    ...
    cmd = ''' /sbin/ifconfig |grep "inet addr:" '''\
        '''|egrep -v "127.0.0.1|10\.|192\.168\." |awk '{print $2}' '''\
        '''|awk -F ":" '{print $2}' '''
    rc, so, se = shell(cmd)
...
def remote_cmd(host, cmd, user="", sshkey="", timeout=10):
    ...
    if sshkey != "":
        cmd = ''' ssh -oConnectTimeout=%s -oStrictHostKeyChecking=no '''\
                '''-i %s %s "%s" ''' % (timeout, sshkey, user_host, cmd)
    else:
        cmd = ''' ssh -oConnectTimeout=%s -oStrictHostKeyChecking=no '''\
                '''%s "%s" ''' % (timeout, user_host, cmd)
...
def transfer_dir(hosts, local_dir, remote_dir, user="", sshkey="", timeout=10):
    ...
    for host in hosts:
        ...
        # 执行传输
        cmd = "scp -r -oConnectTimeout=%s -oStrictHostKeyChecking=no "\
                "%s %s/* %s@%s:%s" % (timeout, sshkey_option, \
